...
def predict_for_query(
    ...
    if feat_df is None:
        _report("DB에 없음 → yt-dlp 수집 시작...", 0.18)
        if dump is None:
            try:
                from yt_dlp_fetcher import fetch_channel_dump
            ...
            def _yt_progress(done: int, total: int) -> None:
                # 0.20 ~ 0.85 구간을 영상 진행률로 채움
                if total <= 0:
                    return
                frac = 0.20 + 0.65 * (done / total)
                _report(f"yt-dlp 영상 수집 {done}/{total}...", frac)

            dump = fetch_channel_dump(query, video_limit=50, on_progress=_yt_progress)
